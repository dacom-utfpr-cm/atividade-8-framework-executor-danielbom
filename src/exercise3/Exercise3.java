...
public class Exercise3 {
    ...
    public static void multiplyMatrixes(double[][] matrixA, double[][] matrixB, double[][] result) {
	...
	if (m != matrixB.length) return false;
	if (n != matrixB[0].length) return false;
    ...
    public static void test(int nThreads, double[][] matrixA, double[][] matrixB) {
	final MultiplyMatrix mult = new MultiplyMatrix(nThreads, matrixA, matrixB);
	mult.run();
	double resultThreads[][] = mult.getResult();
	double resultIterative[][] = MultiplyMatrix.createResultMatrix(matrixA, matrixB);
	...
	System.out.println("Results is equals: " + matrixEquals(resultThreads, resultIterative));
    }
